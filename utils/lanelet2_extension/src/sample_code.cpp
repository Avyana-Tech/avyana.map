...
void loadingAvyanaOSMFile(const std::string & map_file_path)
{
  ...
  // Avyana OSM file parser is registered into lanelet2 library.
  // Therefore, you can used it by just specifying "avyana_osm_handler" parser
  // in load function.
  lanelet_map = lanelet::load(map_file_path, "avyana_osm_handler", projector, &errors);
...
void usingAvyanaTrafficLight(const std::string & map_file_path)
{
  lanelet::LaneletMapPtr lanelet_map;
  lanelet::ErrorMessages errors;
  lanelet::projection::MGRSProjector projector;
  ...
  for (const auto & lanelet : lanelet_map->laneletLayer) {
    // You can access to traffic light element as AvyanaTrafficLight class
    const auto avyana_traffic_lights =
      lanelet.regulatoryElementsAs<lanelet::avyana::AvyanaTrafficLight>();
    for (const auto & light : avyana_traffic_lights) {
      ...
      // Since AvyanaTrafficLight class is inheriting lanelet::TrafficLight
...
int main(int argc, char * argv[])
{
  ...
  loadingAvyanaOSMFile(map_file_path);
  usingMGRSProjector();
  usingAvyanaTrafficLight(map_file_path);
  return 0;
}
