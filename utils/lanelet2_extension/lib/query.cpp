...
#include "lanelet2_extension/math/normalization.hpp"
  ...
  const lanelet::ConstLanelets & lanelets)
{
  ...
  for (const auto & ll : lanelets) {
    std::vector<lanelet::AvyanaTrafficLightConstPtr> ll_tl_re =
      ll.regulatoryElementsAs<lanelet::avyana::AvyanaTrafficLight>();
  ...
  const lanelet::ConstLanelets & lanelets)
{
  ...
  for (const auto & ll : lanelets) {
    std::vector<lanelet::DetectionAreaConstPtr> ll_da_re =
      ll.regulatoryElementsAs<lanelet::avyana::DetectionArea>();
  ...
  const lanelet::ConstLanelets & lanelets)
{
  ...
  for (const auto & ll : lanelets) {
    std::vector<lanelet::NoStoppingAreaConstPtr> ll_no_re =
      ll.regulatoryElementsAs<lanelet::avyana::NoStoppingArea>();
  ...
  ConstLanelet * closest_lanelet_ptr)
{
  ...
  // find by angle
  {
    ...
    for (const auto & llt : candidate_lanelets) {
      ...
      double angle_diff = std::abs(avyana_utils::normalize_radian(segment_angle - pose_yaw));
