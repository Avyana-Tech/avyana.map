...
void printUsage()
{
  std::cout << "Usage:" << std::endl
            << "ros2 run lanelet2_extension avyana_lanelet2_validation"
...
void validateTrafficLight(const lanelet::LaneletMapPtr lanelet_map)
{
  ...
  for (const auto & lanelet : lanelet_map->laneletLayer) {
    ...
    if (avyana_traffic_lights.empty()) {
      continue;
    }
...
int main(int argc, char * argv[])
{
  ...
  auto node = rclcpp::Node::make_shared("avyana_lanelet_validation");
  ...
  lanelet_map = lanelet::load(map_path, "avyana_osm_handler", projector, &errors);
