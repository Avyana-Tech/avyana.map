...
: Node("lanelet2_map_visualization", options)
{
  ...
  sub_map_bin_ = this->create_subscription<avyana_map_messages::msg::HDMapBin>(
...
void Lanelet2MapVisualizationNode::onMapBin(
  const avyana_map_messages::msg::HDMapBin::ConstSharedPtr msg)
{
  ...
  std::vector<lanelet::AvyanaTrafficLightConstPtr> aw_tl_reg_elems =
    lanelet::utils::query::AvyanaTrafficLights(all_lanelets);
  ...
  insertMarkerArray(
    &map_marker_array,
    lanelet::visualization::AvyanaTrafficLightsAsMarkerArray(aw_tl_reg_elems, cl_trafficlights));
  insertMarkerArray(
    &map_marker_array,
